...
## src/proxy.ts (페이지 보호. /admin/* 와 /api 의 관리용 경로)
```ts
...
```
Next.js 15 이하면 파일 이름을 `src/middleware.ts`, 함수 이름을 `middleware` 로 한다. 16부터는 `proxy` 다(`middleware` 는 deprecated). 어느 쪽인지는 00-detect 1번의 버전으로 정한다.
